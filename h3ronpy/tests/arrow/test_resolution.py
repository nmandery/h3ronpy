...
def test_change_resolution_up():
    ...
    for i in range(len(changed)):
        assert h3.get_resolution(changed[i].as_py()) == out_res
...
def test_change_resolution_paired_up():
    ...
    for i in range(changed_df.num_rows):
        assert h3.get_resolution(changed_df["cell_before"][i].as_py()) == 8
        assert h3.get_resolution(changed_df["cell_after"][i].as_py()) == out_res
...
def test_change_resolution_down():
    ...
    assert h3.get_resolution(changed[0].as_py()) == out_res
    assert h3.get_resolution(changed[1].as_py()) == out_res
